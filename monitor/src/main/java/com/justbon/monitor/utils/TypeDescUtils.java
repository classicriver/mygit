...
public final class TypeDescUtils {
    ...
    public static String getMethodParamsDesc(String descriptor) {
        ...
        for (int i = 0; i < descriptor.length() && roundTimes++ <= descriptor.length(); ) {
            ...
            if (isTypeDescriptor(ch)) {
            	...
            	LogFactory.error("TypeDescUtils.processDescriptor(" + descriptor + ") Should never arrive here!!! 0");
                return descriptor;
            }
        }
    ...
    private static int processDescriptor(String descriptor, int idx, StringBuilder sb) {
        ...
        for (; idx < descriptor.length() && roundTimes++ <= descriptor.length(); ++idx) {
            ...
            if (ch == '[') {
                ...
                //理论上永远走不到这里
            	LogFactory.error("TypeDescUtils.processDescriptor(" + descriptor + ", " + idx + ", " + sb + ") Should never arrive here!!! 1");
                return -1;
            }
        }

        //理论上永远走不到这里
        LogFactory.error("TypeDescUtils.processDescriptor(" + descriptor + ", " + idx + ", " + sb + ") Should never arrive here!!! 2");
        return -1;
    }
    ...
    /**
     * @param fullClassName : 形如：java.lang.String
     */
