...
public class DisruptorServerImpl extends AbstractServer {
	...
	private void startDisruptor() {
		...
		ProtocolWorkHandler[] handlers = new ProtocolWorkHandler[Config.getMaxThreads()];
		for(int i =0 ; i< Config.getMaxThreads();i++){
			handlers[i] = new ProtocolWorkHandler(new DefaultAnalysizerImpl(new RocketMQClientImpl()));
		}
		//disruptor.handleEventsWith(handlers);每个handler会单独建立一个线程，链式handler，每条数据都会被所有handler处理
